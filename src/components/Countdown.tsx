// o useEffect é um hook que dispara efeitos colaterais, que é quando algo mudar ou quando algo acontecer, executa alguma função
...
export function Countdown() {
    ...
    // como essas constantes são usadas apenas para fins visuais do próprio component countdown não vai ser usada no contexto
    // para separar os números do minutos e números dos segundos
    // primeiro estou transformando o número em string (de 25 para '25')
    // o slipt divide cada caracter em um array (se tenho '25' ele retorna '2' e '5')
    // mas se eu tiver so '5', não tem como ele repartir, por isso se usa o padStart
    // o padStart verifica se a string não tiver 2 caracteres ele vai preencer o restante para esquerda (por isso start, que é no começo/início) com 0
    // agora se eu tiver '5', ele vai retornar '0' e '5'
    // como o split devolve um array com uma primeira posição e segunda posição, posso desescruturar o array usando os []
    ...
    return (
        <div>
            ...
            {/* como tem mais de uma linha, usar o parêntese por fora dos botões */}
            {hasFinished ? (
                <button
                    disabled
                ...
                >
                    ...
                    // não é possivel colocar uma condicinal dentro de outra condicional sem ter nada em volta, por isso foi coloca as tags vazias <> chamada de fragment, é como se fosse uma div, mas não existe no html
                    <>
                        {/* se está ativo mostrar o botão abandonar ciclo, se não, mostrar o botão iniciar um ciclo */}
                        { isActive ? (
                            <button
                                type="button"
                                // para colocar mais de uma classe tem que concatenar as strings
